 ...
 * 「バクン」に近い発音になる。
 *
 * 生成物は src/audio/<clip>Clip.ts（base64）。別ファイルではなくコードに
 ...
 */
const CLIPS = {
  // 出てくる合図の「ばあっ！」。**「ばあ！」（peek-aboo）から持ってきた設定**。
  // 高く・短くする。長いと出てくる動きに遅れて聞こえる
  baa: { text: 'baaa', pitch: 96, speed: 118, seconds: 0.75, konst: 'BAA_WAV_BASE64', file: 'baaClip.ts' },
  // 叩かれたときの「いてっ」（§4-4）。
  //
  // **痛そうにしないこと**（不変条件11b）。おどけた驚きにする。
  // 魚が痛がって悲しそうに聞こえると、1歳半には
  // 「叩いてはいけないこと」に見える。
  // だから「ばあっ」より**高く・速く・短く**した（pitch 104 / speed 150）。
  // 低くゆっくりだと呻き声になる。
  //
  // 末尾の促音「っ」は、音としては**切れ**なので、
  // `seconds` を短く取って言い終わる前に切っている
  // （「ばあ！」の `kotchi` は語中の促音だったので綴りで閉鎖を作れたが、
  // 語末の「っ」には後ろに閉じる相手が無い）
  ite: { text: 'ite', pitch: 104, speed: 150, seconds: 0.42, konst: 'ITE_WAV_BASE64', file: 'iteClip.ts' },
};


const first = process.argv[2];
const name = first && Number.isNaN(Number(first)) ? first : 'baa';

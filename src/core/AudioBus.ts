 ...
 */
/**
 * 声は2本だけ（設計書 §4-4）。**役割を混ぜないこと。**
 *  - baa … 魚が水面から見えはじめた瞬間。**魚が出た**合図
 *  - ite … 叩かれた瞬間。**当たった**合図
 *
 * 「ばあ！」（peek-aboo）では `kocchi`（空振りのヒント）を持っていたが、
 * このアプリでは**空振りに声を出さない**（§4-5）ので消した。
 */
export type VoiceClip = 'baa' | 'ite';
...
async function loadVoiceBase64(name: VoiceClip): Promise<string> {
  switch (name) {
    ...
    case 'ite': {
      ...
      // チャンクもその文字列ぶんしかない。
      //
      // **まだ録っていない**（Phase 3 で `scripts/make-voice.cjs` が作る）。
      // 無いあいだは空文字が返り、`decodeAudioData` が失敗して
      // `playVoiceClip` が false を返す。**黙って合成音に落ちる**
      // （不変条件7。例外は表に出さない）
      const mod = await import('../audio/iteClip');
      return mod.ITE_WAV_BASE64;
    }
  }
}
...
export class AudioBus {
   ...
   */
  private readonly lastSpeak: Record<VoiceClip, number> = { baa: 0, ite: 0 };
   ...
   * 同じ経路（WebAudio）で鳴らす。
   */
  private readonly voiceBuffers: Record<VoiceClip, AudioBuffer | null> = { baa: null, ite: null };

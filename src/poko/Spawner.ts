...
export class Spawner {
  ...
  /** サプライズが出ているあいだ抽選を止める（§6-3）。`setHeld()` を読むこと */
  private held = false;
  ...
  /**
   * サプライズ（§6-3）が出ているあいだ、新しい魚を出さない。
   *
   * ==========================================================================
   * **`setPaused()` と別の旗にする。** 1つの旗を2箇所から立てると、
   * サプライズが終わった瞬間に**入れ替え待ちの停止まで解けて**、
   * 入れ替えが永遠に終わらなくなる。
   *
   * 止める理由は、サプライズが**画面のいちばん前**に出て
   * 下の段の水たまりを覆うため。実測（412×839）で、下の段の芯は
   * サプライズの当たり楕円の**内側**（0.32 / 0.63）にある。
   * 止めないと、覆われて見えない魚がそこに出て、
   * 「押した相手と違うものが反応する」が起きる。
   *
   * **叩ける相手は途切れない。** 止まっているあいだ、サプライズ自身が
   * 画面でいちばん大きい的として出ている（不変条件4c は「叩ける相手が
   * 居ること」で、水たまりの魚であることは求めていない）。
   * ==========================================================================
   */
  ...
  forceNext(variant: FishVariant | null): void {
    this.forced = variant;
    // ==================================================================
    // **待ち時間も詰める**（2026-09-17、実機で「きんいろが出現しない」と
    // 言われて測った）。抽選の間隔（0.8〜2.0秒）と出はじめの ため・登場が
    // 積み上がって、**押してから 5.4秒**かかっていた。
    // 大人がボタンを押して閉じたあと5秒待たされると、出ていないのと同じに見える。
    // **上限（`MAX_UP_FISH`）は破らない** —— 破ると §4-2 が壊れる
    // ==================================================================
    if (variant) this.wait = 0;
  }

  /** 決め打ちがまだ残っているか（`App` が入れ替えをまたいで持ち直すために見る） */
  hasForced(): boolean {
  ...
  update(dt: number, fish: FishSystem): void {
    ...
    // **決め打ちがあるのに席が空いていないなら、1匹ぶん先に沈ませる**（§6-2）。
    // 待つと 6〜8秒かかって「出現しない」に見える（`FishSystem.retreatOldest()`）
    // **2匹出ているときだけ。** 1匹しか出ていないのに沈ませると、
    // 叩ける相手が 0 になる（不変条件4c）
    if (this.forced && fish.countActive() >= MAX_UP_FISH && fish.countRisingOrUp() >= 2) {
      fish.retreatOldest();
    }
    // ==================================================================

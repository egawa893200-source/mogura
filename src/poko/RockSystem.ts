 ...
 *
 * **岩の奥。** `visible = false` で消してはいるが、出はじめの1フレームで
 * 岩の手前に現れると「ぱっと湧いた」に見えるので、奥から出す
 */
const HIDDEN_Z = -0.55;
...
export class RockSystem {
  ...
  private buildRock(config: RockConfig): THREE.Mesh {
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      // **奥行きを潰す。** 真球だと「岩」ではなく岩石惑星に見える。
      // カメラは正面固定なので、薄くしても輪郭は変わらない
      pos.setXYZ(i, x * n, y * n * 0.88, z * n * 0.55);
    }
  ...
  private place(): void {
    ...
    // **少し下がりながら出てくる**（2026-09-14、絵を見て直した）。
    // 上げていたら、出きったところで**岩とぴったり重なって**輪郭が消えた。
    // 「岩陰から」なので、岩の下端にかかりながら手前へ来るのが正しい
    fish.group.position.set(
      runtime.worldPosition.x,
      runtime.worldPosition.y - 0.12 * eased,
